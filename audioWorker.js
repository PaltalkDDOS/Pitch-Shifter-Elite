...
/**
 * Advanced MemoryManager with Manacher-inspired gap management, WebGPU support, 
 * adaptive defragmentation, predictive allocation, and LRU caching for efficient 
 * audio buffer allocation, synchronized with jungle.js for HiFi audio.
 */
class MemoryManager {
	constructor(maxSize, options = {}) {
		...
		this.buffers = new Map(); // LRU cache for additional buffer storage
		this.accessTimes = new Map(); // Tracks access times for LRU
		this.priorityWeights = new Map(); // Tracks priority weights for LRU
		this.sharedCache = typeof SharedArrayBuffer !== "undefined" ? new SharedArrayBuffer(1024 * 1024) : null; // For multi-threading
		this.webGPUDevice = null; // WebGPU support
		this.maxGPUBufferSize = 1024 * 1024 * 16; // 16MB for GPU
	}

	/**
	 * Initializes WebGPU support for MemoryManager.
	 * @param {Object} webGPUDevice - WebGPU device instance
	 */
	...
	/**
	 * Expands the memory pool with predictive sizing based on allocation history and spectralProfile.
	 * @param {number} newSize - Desired new size
	 * @param {Object} spectralProfile - Spectral profile for audio optimization
	 */
	expandPool(newSize, spectralProfile = {}) {
		...
		try {
			const avgAllocation = this.allocationHistory.length > 0 ?
				...
				newSize;
			// Adjust size based on spectralProfile (e.g., bassHeavy needs larger buffers)
			const profileFactor = spectralProfile.bass > 0.7 || spectralProfile.transientEnergy > 0.7 ? 2.0 : 1.5;
			const targetSize = Math.max(this.maxSize * profileFactor, newSize, avgAllocation * 2);
			const newPool = new Float32Array(Math.ceil(targetSize));
			newPool.set(this.pool);
			this.pool = newPool;
			this.maxSize = newPool.length;
			console.debug(`Expanded pool to ${this.maxSize} (requested: ${newSize}, predicted: ${targetSize}, profileFactor: ${profileFactor})`);
	...
	/**
	 * Allocates a buffer using Manacher-inspired gap analysis, WebGPU, and LRU caching.
	...
	allocate(size, spectralProfile = {}) {
		...
		// Adjust size based on devicePerf and spectralProfile
		const devicePerf = spectralProfile.devicePerf || "medium";
		const adjustedSize = devicePerf === "low" ? Math.min(size, this.maxSize / 2) :
			devicePerf === "high" ? Math.max(size, this.maxSize / 4) : size;

		// WebGPU allocation for large buffers
		if (this.webGPUDevice && adjustedSize > 1024 && adjustedSize <= this.maxGPUBufferSize && spectralProfile.transientEnergy > 0.7) {
			try {
				...
				const buffer = new Float32Array(new ArrayBuffer(adjustedSize));
				buffer.gpuBuffer = gpuBuffer; // Track GPU buffer
				return buffer;
			} catch (error) {
				console.warn(`GPU buffer allocation failed: ${error.message}, falling back to CPU`);
				this.performanceMetrics.webGPUFallbacks = (this.performanceMetrics.webGPUFallbacks || 0) + 1;
			}
		}

		// Reuse from bufferPool (Manacher-inspired)
		if (this.bufferPool.has(adjustedSize)) {
			const availableBuffers = this.bufferPool.get(adjustedSize);
			for (let i = 0; i < availableBuffers.length; i++) {
				...
				if (this.isBufferValid(buffer)) {
					...
					this.allocations.set(buffer.offset, adjustedSize);
		...
		// LRU cache check
		const cacheKey = `buffer_${adjustedSize}_${spectralProfile.vocalPresence || 0.5}_${spectralProfile.bass || 0.5}`;
		let buffer = this.getBuffer(cacheKey);
		if (buffer) {
			this.performanceMetrics.cacheHits++;
			return buffer;
		}

		// Manacher-inspired gap analysis
		const gapIndex = this.findBestGap(adjustedSize);
		if (gapIndex !== -1) {
			const [gapOffset, gapSize] = this.freeGaps[gapIndex];
			this.allocations.set(gapOffset, adjustedSize);
			this.bufferTimestamps.set(gapOffset, Date.now());
			if (gapSize > adjustedSize) {
				this.freeGaps[gapIndex] = [gapOffset + adjustedSize, gapSize - adjustedSize];
			} else {
				this.freeGaps.splice(gapIndex, 1);
			}
			buffer = this.pool.subarray(gapOffset, gapOffset + adjustedSize);
			buffer.offset = gapOffset;
			this.performanceMetrics.allocations++;
		} else {
			// Adaptive defragmentation
			...
			if (this.nextOffset + adjustedSize > this.maxSize) {
				this.expandPool(this.nextOffset + adjustedSize, spectralProfile);
			}

			// Allocate new block
			const offset = this.nextOffset;
			this.allocations.set(offset, adjustedSize);
			this.bufferTimestamps.set(offset, Date.now());
			this.nextOffset += adjustedSize;
			buffer = this.pool.subarray(offset, offset + adjustedSize);
			buffer.offset = offset;
			this.performanceMetrics.allocations++;
		}

		// Store in LRU cache
		this.allocateBuffer(cacheKey, buffer);
		this.performanceMetrics.processingTime = (this.performanceMetrics.processingTime || 0) + (performance.now() - startTime);
		self.postMessage({
	...
	/**
	 * Predicts if defragmentation is needed based on allocation patterns and spectralProfile.
	 * @param {Object} spectralProfile - Spectral profile for audio context
	 * @returns {boolean} True if defragmentation is recommended
	 */
	shouldDefragment(spectralProfile = {}) {
		...
		// Increase defragmentation for bassHeavy or vocal profiles
		const profileFactor = spectralProfile.bass > 0.7 || spectralProfile.vocalPresence > 0.7 ? 0.8 : 1.0;
		return variance > 1000 * profileFactor;
	}
	...
	/**
	 * Retrieves a buffer from LRU cache.
	 * @param {string} key - Cache key
	 * @returns {Float32Array|null} Cached buffer or null
	 */
	getBuffer(key) {
		const startTime = performance.now();
	...
	/**
	 * Allocates a buffer in LRU cache.
	 * @param {string} key - Cache key
	 * @param {Float32Array} value - Buffer to store
	 */
	allocateBuffer(key, value) {
		const startTime = performance.now();
		this.buffers.set(key, value);
		this.accessTimes.set(key, Date.now());
		this.priorityWeights.set(key, value.length || 1);
		this.performanceMetrics.allocations++;
	...
	/**
	 * Prunes LRU cache based on max entries.
	...
	pruneCache(maxEntries) {
		...
		this.performanceMetrics.pruneCount++;
		this.performanceMetrics.processingTime = (this.performanceMetrics.processingTime || 0) + (performance.now() - startTime);
		self.postMessage({
			event: "cache_pruned",
			removed: sortedKeys.length,
			metrics: this.performanceMetrics
		});
	}

	/**
	 * Auto-tunes cache size based on hit rate and allocation rate.
	 */
	autoTuneCache() {
		const hitRate = this.performanceMetrics.cacheHits / (this.performanceMetrics.cacheHits + this.performanceMetrics.cacheMisses || 1);
		...
		} else if (this.performanceMetrics.allocations > 1000 && this.buffers.size < this.maxSize / 2) {
 ...
 * WebGPU support, and noise-aware processing, synchronized with jungle.js for HiFi audio.
 */
class OptimizedFFT {
	constructor(size, memoryManager, useWebGPU, webGPUDevice, spectralProfile = {}) {
		...
		this.fftBuffer = memoryManager.allocate(size * 2, spectralProfile);
	...
	selectWindowType() {
		const {
			transientEnergy = 0.5, vocalPresence = 0.5, currentGenre = "Pop", spectralFlatness = 0.5
		} = this.spectralProfile;
		if (this.spectralProfile.profile === "bassHeavy" || transientEnergy > 0.8 || ["DrumAndBass", "Techno", "EDM"].includes(currentGenre)) {
			return "kaiser"; // Strong transient preservation
		}
		if (this.spectralProfile.profile === "vocal" || this.spectralProfile.profile === "karaokeDynamic" || vocalPresence > 0.7 || currentGenre === "Karaoke") {
			return "hamming"; // Smooth vocal formant preservation
		}
		if (this.spectralProfile.profile === "warm" || this.spectralProfile.profile === "proNatural" || currentGenre === "Classical" || spectralFlatness > 0.7) {
			return "blackman-harris"; // Warmth, minimal leakage
		}
		if (this.spectralProfile.profile === "rockMetal") {
			return "nuttall"; // High transient clarity
		}
		return "hann"; // Default, balanced
	}

	precomputeTwiddles(size) {
		const twiddles = this.memoryManager.allocate(size * 2, this.spectralProfile);
		try {
			const angleStep = -2 * Math.PI / size;
			for (let k = 0; k < size; k += 16) { // Vectorized x16
				for (let i = 0; i < 16 && k + i < size; i++) {
					const angle = angleStep * (k + i);
					twiddles[(k + i) * 2] = ensureFinite(Math.cos(angle));
					twiddles[(k + i) * 2 + 1] = ensureFinite(Math.sin(angle));
				}
			}
			...
			handleError("Failed to precompute twiddles", error, {
				size
			});
			throw error;
		}
	...
	precomputeBitReversal(size) {
		...
		try {
			...
			handleError("Failed to precompute bit-reversal", error, {
				size
			});
			throw error;
		}
	...
	precomputeWindow(size, type) {
		const window = this.memoryManager.allocate(size, this.spectralProfile);
		try {
				...
				for (let j = 0; j < 16 && i + j < size; j++) {
					...
					if (type === "blackman-harris") {
						const a0 = 0.35875,
							...
							a2 = 0.14128,
							a3 = 0.01168;
						window[i + j] = ensureFinite(a0 - a1 * Math.cos(t) + a2 * Math.cos(2 * t) - a3 * Math.cos(3 * t));
					} else if (type === "kaiser") {
						const beta = this.spectralProfile.transientEnergy > 0.8 ? 12.0 :
							this.spectralProfile.transientEnergy > 0.6 ? 10.0 : 8.0;
						const bessel = (x) => {
							let sum = 1,
								term = 1,
								n = 1;
						...
						window[i + j] = ensureFinite(bessel(beta * Math.sqrt(1 - (2 * (i + j) / (size - 1) - 1) ** 2)) / bessel(beta));
						...
						window[i + j] = ensureFinite(0.5 * (1 - Math.cos(t)));
					} else {
						window[i + j] = ensureFinite(0.54 - 0.46 * Math.cos(t)); // Hamming
					}
				}
			}
			...
			handleError("Failed to precompute window", error, {
				size,
				type
			});
			throw error;
		...
		try {
			const shaderCode = `
                ...
                @compute fn fft(@builtin(global_invocation_id) id: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
                    ...
                    // Vocal formant preservation
                    if (config.vocalPresence > 0.7 && idx < halfSize) {
                        output[i].x = output[i].x * 1.1; // Boost vocal formants
                    }
                    // Bass harmonic enhancement
                    if (config.transientEnergy > 0.8 && idx < 32) {
                        output[i].x = output[i].x * 1.2; // Boost low-end transients
                    }
                }
            `;
			...
			handleError("Failed to initialize WebGPU FFT", error, {
				size: this.size
			});
	...
	async fft(signal, noiseInfo = {
		...
		if (!signal || signal.length !== this.size) {
			handleError("Invalid signal length", new Error(`Expected ${this.size}, got ${signal?.length || 0}`));
		...
		if (this.useWebGPU && this.webGPUDevice && this.webGPUPipeline) {
			try {
				...
				handleError("FFT WebGPU failed, falling back to CPU", error, {
					size: this.size
				});
		...
		if (this.spectralProfile.profile === "bassHeavy" || this.spectralProfile.transientEnergy > 0.8) {
			for (let i = 0; i < 32; i++) { // Boost low-end harmonics
				result[i * 2] *= 1.2;
			}
		}
		if (this.spectralProfile.profile === "vocal" || this.spectralProfile.vocalPresence > 0.7) {
			for (let i = 32; i < 128; i++) { // Preserve vocal formants
				result[i * 2] *= 1.1;
			}
		}
	...
	async fftBatch(signals, noiseInfo = {
		type: "white",
		confidence: 0.5
	}, suppressArtifacts = () => ({
		output: signals[0]
		...
		if (!Array.isArray(signals) || signals.some(s => !s || s.length !== this.size)) {
			handleError("Invalid batch signals", new Error(`Must be array of Float32Array with length ${this.size}`));
		...
		if (this.useWebGPU && this.webGPUDevice && this.webGPUPipeline) {
			try {
				...
				handleError("FFT WebGPU batch failed, falling back to CPU", error, {
					batchSize,
					size: this.size
				});
				this.performanceMetrics.webGPUFallbacks++;
		...
		// Apply profile-based enhancements for batch
	...
	async fftCPU(signal) {
		...
		for (let i = 0; i < this.size; i += 16) { // Vectorized x16
			for (let j = 0; j < 16 && i + j < this.size; j++) {
				result[(i + j) * 2] = ensureFinite(signal[i + j] * this.windowCache[i + j]);
			}
		}
		...
		for (let size = 2; size <= this.size; size *= 2) {
			...
			for (let i = 0; i < this.size; i += size) {
				for (let j = i, k = 0; j < i + halfSize; j += 16, k += tableStep * 16) {
					for (let m = 0; m < 16 && j + m < i + halfSize; m++) {
						const c = this.twiddles[(k + m * tableStep) * 2];
						const s = this.twiddles[(k + m * tableStep) * 2 + 1];
						let tReal = result[(j + m + halfSize) * 2] * c - result[(j + m + halfSize) * 2 + 1] * s;
						let tImag = result[(j + m + halfSize) * 2] * s + result[(j + m + halfSize) * 2 + 1] * c;
						result[(j + m + halfSize) * 2] = ensureFinite(result[(j + m) * 2] - tReal);
						result[(j + m + halfSize) * 2 + 1] = ensureFinite(result[(j + m) * 2 + 1] - tImag);
						result[(j + m) * 2] = ensureFinite(result[(j + m) * 2] + tReal);
						result[(j + m) * 2 + 1] = ensureFinite(result[(j + m) * 2 + 1] + tImag);
					}
				}
			}
		}
	...
	async fftWebGPUBatch(signals, numChannels = 1) {
		if (!this.webGPUDevice || !this.webGPUPipeline) {
			handleError("WebGPU not initialized", new Error("WebGPU pipeline missing"));
		...
		for (let b = 0; b < batchSize; b++) {
			for (let c = 0; c < numChannels; c++) {
				for (let i = 0; i < this.size; i++) {
					inputData[(b * numChannels * this.size + c * this.size + i) * 2] = Math.f16(ensureFinite(signals[b][i]));
				}
			}
		}
		inputBuffer.unmap();
		...
		const configBuffer = this.webGPUDevice.createBuffer({
			size: 28, // Increased for vocalPresence, transientEnergy
			usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
			mappedAtCreation: true
		});
		const configData = new DataView(configBuffer.getMappedRange());
		configData.setUint32(0, this.size, true);
		configData.setUint32(4, this.size, true);
		configData.setUint32(8, Math.log2(this.size), true);
		configData.setUint32(12, batchSize, true);
		configData.setUint32(16, numChannels, true);
		configData.setFloat32(20, this.spectralProfile.vocalPresence || 0.5, true);
		configData.setFloat32(24, this.spectralProfile.transientEnergy || 0.5, true);
		configBuffer.unmap();
		...
		const bindGroup = this.webGPUDevice.createBindGroup({
			...
			entries: [{
				...
				},
				{
					binding: 1,
					resource: {
						buffer: buffers.twiddlesBuffer
					}
				},
				{
					binding: 2,
					resource: {
						buffer: buffers.outputBuffer
					}
				},
				{
					binding: 3,
		...
		try {
			...
			for (let b = 0; b < batchSize; b++) {
				const result = this.memoryManager.allocate(this.size * 2, this.spectralProfile);
				for (let c = 0; c < numChannels; c++) {
					for (let i = 0; i < this.size; i++) {
						result[i * 2] = ensureFinite(Math.f32(outputData[(b * numChannels * this.size + c * this.size + i) * 2]));
						result[i * 2 + 1] = ensureFinite(Math.f32(outputData[(b * numChannels * this.size + c * this.size + i) * 2 + 1]));
					}
				}
				results.push(result);
			}
			...
			handleError("Failed to read WebGPU batch output", error, {
				batchSize,
				size: this.size
			});
			throw error;
	...
	createWebGPUBatchBuffers(signals, numChannels) {
		...
		for (let i = 0; i < this.twiddles.length; i++) {
			twiddlesData[i] = Math.f16(ensureFinite(this.twiddles[i]));
		}
	...
	async ifft(complexData, noiseInfo = {
		type: "white",
		confidence: 0.5
	}) {
		if (!complexData || complexData.length !== this.size * 2) {
			handleError("Invalid complexData length", new Error(`Expected ${this.size * 2}, got ${complexData?.length || 0}`));
			throw new Error(`Invalid complexData length: ${complexData?.length || 0} (expected ${this.size * 2})`);
		}

		const buffer = this.memoryManager.allocate(this.size * 2, this.spectralProfile);
		try {
			for (let i = 0; i < this.size; i += 16) {
				for (let j = 0; j < 16 && i + j < this.size; j++) {
					buffer[(i + j) * 2] = ensureFinite(complexData[(i + j) * 2]);
					buffer[(i + j) * 2 + 1] = ensureFinite(-complexData[(i + j) * 2 + 1]);
				}
			}

			const fftResult = await this.fft(buffer, noiseInfo);
			const waveletFilter = this.applyWaveletFilter(fftResult, noiseInfo);
			const output = this.memoryManager.allocate(this.size, this.spectralProfile);
			for (let i = 0; i < this.size; i += 16) {
				for (let j = 0; j < 16 && i + j < this.size; j++) {
					output[i + j] = ensureFinite(waveletFilter[(i + j) * 2] / this.size);
				}
			}
			...
			this.memoryManager.free(buffer);
			handleError("Failed to compute IFFT", error, {
				size: this.size
			});
			throw error;
		}
	...
	applyWaveletFilter(signal, noiseInfo) {
		const output = this.memoryManager.allocate(signal.length, this.spectralProfile);
		...
		// Profile-based wavelet adjustments
		if (this.spectralProfile.profile === "vocal" || this.spectralProfile.vocalPresence > 0.7) {
			for (let i = 32; i < 128; i++) {
				output[i * 2] *= 1.1; // Preserve vocal formants
			}
		}
		return output;
	}
	...
	computeSignalHash(signal) {
		...
		for (let i = 0; i < signal.length; i += step) {
			hash = (hash * 31 + Math.round(ensureFinite(signal[i]) * 10000)) | 0;
		}
	...
	autoTuneParameters() {
		...
		if (avgTime > 8 && this.size > 512 && this.spectralProfile.devicePerf === "low") {
			this.size = Math.max(256, this.size / 2);
			console.debug(`Auto-tuned FFT size to ${this.size} due to high latency on low device`);
			this.reinitialize();
	...
	reinitialize() {
		this.dispose();
		this.fftBuffer = this.memoryManager.allocate(this.size * 2, this.spectralProfile);
		...
		this.initializeWebGPU().catch(err => handleError("WebGPU reinitialization failed", err, {
			size: this.size
		}));
	}
...
function phaseVocoder(timeData, pitchMult, timeMult, sampleRate, fftInstance, performanceLevel = "high", spectralProfile = {}) {
	...
	async function processFrameBatch(frames, channelIdx) {
		const fftResults = await fftInstance.fftBatch(frames, {
			type: "white",
			confidence: 0.5
		}, (signal, profile, noiseInfo, mem, rms) => {
	...
	for (let frame = 0; frame < numFrames && outputPos < outputLength; frame += batchSize) {
		...
		for (let ch = 0; ch < numChannels; ch++) {
			...
			for (let b = 0; b < frames.length; b++) {
				...
				const {
					magnitudes,
					phases
				} = getMagnitudeAndPhase(fftData, fftSize, 0.85);
				...
				let synthTimeData;
				try {
					synthTimeData = await fftInstance.ifft(synthFrame, {
						type: "white",
						confidence: 0.5
					});
				} catch (error) {
		...
		self.postMessage({
			...
			frame: frame + batchSize - 1,
			isTransient: lastTransient,
			outputPos,
	...
	for (let i = 1; i < fftSize / 2; i++) {
		if (magnitudes[i] > (spectralProfile.rms || 0.1) * 2 && magnitudes[i] > magnitudes[i - 1] && magnitudes[i] > magnitudes[i + 1]) {
			pitches.push({
				...
				confidence: magnitudes[i] / (spectralProfile.rms || 0.1)
			});
		}
	}
...
function timeFrequencyReassignment(
	...
	return {
		frequencies: reassignedFreqs,
		times: reassignedTimes
	};
}
...
class OnsetDetector {
	 ...
	 */
	async detect(magnitudes, prevMagnitudes, spectralProfile, noiseInfo = {
		type: "white",
		confidence: 0.5
	}, suppressArtifacts = () => ({
		output: magnitudes
		...
		const {
			forgetGate: forgetGateBack,
			inputGate: inputGateBack,
			cellGate: cellGateBack,
			outputGate: outputGateBack
		} = backwardResult;
		for (let i = 0; i < this.hiddenSize; i++) {
...
/**
 * Spectral analyzer with mel filter banks, transformer, and instrument detection,
 * optimized for HiFi audio, synchronized with jungle.js and OptimizedFFT.js.
 */
class SpectralAnalyzer {
	constructor(sampleRate, fftSize = 2048, devicePerf = "medium", memoryManager) {
		if (!Number.isInteger(fftSize) || fftSize < 16 || fftSize > 262144 || (fftSize & (fftSize - 1)) !== 0) {
			handleError("Invalid FFT size", new Error(`FFT size ${fftSize} must be a power of 2, 16 <= size <= 262144`));
			throw new Error(`Invalid FFT size: ${fftSize}`);
		}
		if (!memoryManager) {
			handleError("MemoryManager required", new Error("MemoryManager is null"));
		...
		this.memoryManager = memoryManager;
		this.numFilters = devicePerf === "high" ? 48 : devicePerf === "medium" ? 32 : 24;
		...
		this.instrumentSignatures = {
			guitar: {
				range: [200, 800],
				harmonicBoost: 1.5,
				template: this.generateTemplate([200, 800])
			},
			piano: {
				range: [1000, 4000],
				harmonicBoost: 1.3,
				template: this.generateTemplate([1000, 4000])
			},
			violin: {
				range: [2000, 6000],
				harmonicBoost: 1.4,
				template: this.generateTemplate([2000, 6000])
			},
			drums: {
				range: [2000, 8000],
				transientBoost: 1.5,
				template: this.generateTemplate([2000, 8000])
			},
			bass: {
				range: [60, 250],
				harmonicBoost: 1.6,
				template: this.generateTemplate([60, 250])
			}
		};
		...
		this.fft = new OptimizedFFT(fftSize, memoryManager, true, null, {
			devicePerf
		});
	}

	initTransformer(devicePerf) {
		const numHeads = devicePerf === "high" ? 6 : devicePerf === "medium" ? 4 : 2;
		const dModel = devicePerf === "high" ? 64 : devicePerf === "medium" ? 48 : 32;
		...
		return {
			...
			numHeads,
			dModel,
			dropout: devicePerf === "low" ? 0.2 : 0.1
		};
	}
	...
	generateTemplate(range) {
		...
		try {
			for (let i = 0; i < this.fftSize / 2; i += 16) { // Vectorized x16
				for (let j = 0; j < 16 && i + j < this.fftSize / 2; j++) {
					const freq = (i + j) * freqPerBin;
					if (freq >= range[0] && freq <= range[1]) {
						template[i + j] = ensureFinite(1 / (1 + Math.abs(freq - (range[0] + range[1]) / 2) / 100));
					} else {
						template[i + j] = 0;
					}
				}
			...
			this.memoryManager.free(template);
			handleError("Failed to generate instrument template", error, {
				range
			});
			throw error;
		}
	...
	initMelFilterBanks(numFilters, minFreq, maxFreq) {
		...
		try {
			for (let i = 0; i < melPoints.length; i += 16) { // Vectorized x16
				for (let j = 0; j < 16 && i + j < melPoints.length; j++) {
					melPoints[i + j] = ensureFinite(melMin + (melMax - melMin) * (i + j) / (numFilters + 1));
				}
			}
			const freqPoints = melPoints.map(m => ensureFinite(700 * (10 ** (m / 2595) - 1)));
			...
			for (let i = 1; i <= numFilters; i++) {
				...
				for (let j = bins[i - 1]; j < bins[i + 1]; j++) {
					if (j < bins[i]) filter[j] = ensureFinite((j - bins[i - 1]) / (bins[i] - bins[i - 1]));
					else filter[j] = ensureFinite((bins[i + 1] - j) / (bins[i + 1] - bins[i]));
				}
				filters.push(filter);
			}
			...
			this.memoryManager.free(melPoints);
			handleError("Failed to initialize mel filter banks", error, {
				numFilters,
				minFreq,
				maxFreq
			});
	...
	softmax(x) {
		...
		const exp = x.map(v => Math.exp(ensureFinite(v - max)));
		const sum = exp.reduce((a, b) => a + b, 0);
		return exp.map(v => ensureFinite(v / sum));
	}

	relu(x) {
		return Math.max(0, ensureFinite(x));
	}
	...
	layerNorm(input, gamma, epsilon = 1e-5) {
		...
		for (let i = 0; i < input.length; i += 16) { // Vectorized x16
			for (let j = 0; j < 16 && i + j < input.length; j++) {
				output[i + j] = ensureFinite(gamma[i + j] * (input[i + j] - mean) / Math.sqrt(variance + epsilon));
			}
		}
		return output;
	}
	...
	positionalEncoding(length, dModel) {
		...
		for (let pos = 0; pos < length; pos++) {
			for (let i = 0; i < dModel; i += 16) { // Vectorized x16
				for (let j = 0; j < 16 && i + j < dModel; j++) {
					const angle = pos / Math.pow(10000, (2 * (i + j)) / dModel);
					pe[pos * dModel + i + j] = ensureFinite((i + j) % 2 === 0 ? Math.sin(angle) : Math.cos(angle));
				}
			}
		}
		return pe;
	...
	transformerForward(features, spectralProfile = {}) {
		const startTime = performance.now();
		const {
			layers,
			numHeads,
		...
		try {
			for (let i = 0; i < dModel && i < features.length; i++) {
				input[i] = ensureFinite(features[i]);
			}

			const pe = this.positionalEncoding(1, dModel);
			for (let i = 0; i < dModel; i++) {
				input[i] = ensureFinite(input[i] + pe[i]);
			}
			...
			for (const layer of layers) {
				...
				// Vectorized matrix multiplication x16
				for (let i = 0; i < dModel; i += 16) {
					for (let j = 0; j < 16 && i + j < dModel; j++) {
						let qSum = 0,
							kSum = 0,
							vSum = 0;
						for (let m = 0; m < dModel; m += 16) {
							for (let n = 0; n < 16 && m + n < dModel; n++) {
								qSum += input[m + n] * layer.weights.q[(m + n) * dModel + (i + j)];
								kSum += input[m + n] * layer.weights.k[(m + n) * dModel + (i + j)];
								vSum += input[m + n] * layer.weights.v[(m + n) * dModel + (i + j)];
							}
						}
						q[i + j] = ensureFinite(qSum);
						k[i + j] = ensureFinite(kSum);
						v[i + j] = ensureFinite(vSum);
					}
				}
				...
				for (let h = 0; h < numHeads; h++) {
					...
					// Vectorized attention scores x16
					for (let i = 0; i < headSize; i += 16) {
						for (let j = 0; j < 16 && i + j < headSize; j++) {
							let sum = 0;
							for (let m = 0; m < headSize; m += 16) {
								for (let n = 0; n < 16 && m + n < headSize; n++) {
									sum += q[start + i + j] * k[start + m + n];
								}
							}
							scores[i + j] = ensureFinite(sum / Math.sqrt(headSize));
						}
					}
					const attnWeights = this.softmax(scores);
					for (let i = 0; i < headSize; i += 16) {
						for (let j = 0; j < 16 && i + j < headSize; j++) {
							for (let m = 0; m < headSize; m++) {
								attention[start + i + j] += ensureFinite(attnWeights[m] * v[start + m] * (Math.random() < dropout ? 0 : 1));
							}
						}
					}
				...
				if (spectralProfile.profile === "bassHeavy" || spectralProfile.transientEnergy > 0.8) {
					for (let i = 0; i < dModel / 4; i++) {
						attention[i] *= 1.2; // Boost low-end features
					}
				}
				if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
					for (let i = dModel / 4; i < dModel / 2; i++) {
						attention[i] *= 1.1; // Preserve vocal features
					}
				}

				for (let i = 0; i < dModel; i++) {
					input[i] = ensureFinite(input[i] + attention[i]);
				}
				...
				for (let i = 0; i < ff.length; i += 16) {
					for (let j = 0; j < 16 && i + j < ff.length; j++) {
						let sum = layer.biases.ff1[i + j];
						for (let m = 0; m < dModel; m++) {
							sum += input[m] * layer.weights.ff1[m * ff.length + (i + j)];
						}
						ff[i + j] = this.relu(sum);
					}
				}
				...
				for (let i = 0; i < dModel; i += 16) {
					for (let j = 0; j < 16 && i + j < dModel; j++) {
						let sum = layer.biases.ff2[i + j];
						for (let m = 0; m < ff.length; m++) {
							sum += ff[m] * layer.weights.ff2[m * dModel + (i + j)];
						}
						output[i + j] = ensureFinite(sum * (Math.random() < dropout ? 0 : 1));
					}
				}

				for (let i = 0; i < dModel; i++) {
					input[i] = ensureFinite(input[i] + output[i]);
				}
			...
			handleError("Transformer forward pass failed", error, {
				dModel
			});
			throw error;
		}
	...
	async loadAndAnalyzeJun(fileData, spectralProfile = {
		devicePerf: this.devicePerf
	}) {
		const startTime = performance.now();
		if (!fileData || !(fileData instanceof ArrayBuffer)) {
			handleError("Invalid JUN file data", new Error("File data must be an ArrayBuffer"));
		...
		try {
			...
			if (sampleRate !== this.sampleRate) {
				handleError("Sample rate mismatch", new Error(`JUN file sample rate (${sampleRate}) does not match analyzer (${this.sampleRate})`));
			...
			for (let w = 0; w < workerCount; w++) {
				...
				const worker = new Worker(URL.createObjectURL(new Blob([`
                    importScripts("OptimizedFFT.js"); // Assume OptimizedFFT is available
                    self.onmessage = async ({ data }) => {
                        const { samples, start, end, fftSize, sampleRate, channel, spectralProfile } = data;
                        const fft = new OptimizedFFT(fftSize, { allocate: size => new Float32Array(size) }, true, null, spectralProfile);
                        ...
                        for (let offset = start; offset < end; offset += fftSize) {
                            ...
                            const fftData = await fft.fft(timeData, { type: "white", confidence: 0.5 }, suppressArtifacts);
                            const { magnitudes, phases } = getMagnitudeAndPhase(fftData, fftSize, spectralProfile, { type: "white", confidence: 0.5 }, { allocate: size => new Float32Array(size) }, 0.5);
                            results.push({ rms, timeData: Array.from(timeData), magnitudes, phases });
                        }
			...
			for (const chunk of results) {
				...
				const profile = this.analyze(magnitudes, phases, timeData, rms, channels === 2 ? "mono" : "mono", spectralProfile);
			...
			handleError("JUN file processing failed", error, {
				fftSize: this.fftSize
			});
			throw error;
		}
	...
	analyze(magnitudes, phases, timeData, rms, channel = "mono", spectralProfile = {}) {
		const startTime = performance.now();
		const freqPerBin = this.sampleRate / this.fftSize;
		let subBass = 0,
			bass = 0,
		...
		try {
			...
			for (let i = 0; i < this.fftSize / 2; i += 16) { // Vectorized x16
				for (let j = 0; j < 16 && i + j < this.fftSize / 2; j++) {
					const diff = magnitudes[i + j] - (this.lastMagnitudes[i + j] || 0);
					spectralFlux += diff > 0 ? Math.log1p(ensureFinite(diff)) : 0;
				}
			}
			...
			for (const [instr, {
					template,
					harmonicBoost
				}] of Object.entries(this.instrumentSignatures)) {
				instrumentEnergies[instr] = 0;
				for (let i = 0; i < this.fftSize / 2; i += 16) {
					for (let j = 0; j < 16 && i + j < this.fftSize / 2; j++) {
						if (template[i + j] > 0) {
							instrumentEnergies[instr] += ensureFinite(magnitudes[i + j] * template[i + j] * (spectralProfile.profile === "bassHeavy" && instr === "bass" ? 1.2 : 1));
						}
					}
				}
			}
			...
			for (let i = 0; i < this.fftSize / 2; i += 16) {
				for (let j = 0; j < 16 && i + j < this.fftSize / 2; j++) {
					const freq = (i + j) * freqPerBin;
					const energy = ensureFinite(magnitudes[i + j] * magnitudes[i + j]);
					totalEnergy += energy;
					...
					if (i + j > 0 && i + j < this.fftSize / 2 - 1) {
						const magDiff = magnitudes[i + j] - (magnitudes[i + j - 1] + magnitudes[i + j + 1]) / 2;
			...
			const melEnergies = this.melFilterBanks.map(bank => {
				...
				for (let i = 0; i < this.fftSize / 2; i += 16) {
					for (let j = 0; j < 16 && i + j < this.fftSize / 2; j++) {
						sum += ensureFinite(magnitudes[i + j] * bank[i + j] * (spectralProfile.profile === "vocal" && i + j * freqPerBin > 300 && i + j * freqPerBin < 3400 ? 1.1 : 1));
					}
				}
				return sum > 0 ? ensureFinite(20 * Math.log10(sum)) : -Infinity;
			});
			const mfcc = this.dct(melEnergies).slice(0, 13);

			// Chromagram
			this.computeChromagram(magnitudes, phases, spectralProfile);
			...
			const instrumentConfidences = this.transformerForward(features, spectralProfile);
			...
			const profile = {
				...
				vocalPresence: Math.min(1, ensureFinite(formantPeaks / 10 + vocalEnergy / (totalEnergy || 1))),
				transientEnergy: Math.min(1, ensureFinite(transientPeaks / 15 + spectralFlux / (totalEnergy || 1))),
				mfcc,
				spectralFlatness,
				...
				zeroCrossingRate,
				spectralCentroid,
				currentGenre: this.classifyGenre(profile, spectralProfile),
				profile: spectralProfile.profile || "smartStudio"
			};
			...
			return this.smoothProfile(profile, detectedInstruments, spectralProfile);
		} catch (error) {
			handleError("Spectral analysis failed", error, {
				fftSize: this.fftSize,
				channel
			});
			throw error;
	...
	computeChromagram(magnitudes, phases, spectralProfile = {}) {
		const freqPerBin = this.sampleRate / this.fftSize;
		...
		try {
					...
					for (let n = 0; n < 12; n++) {
						...
						for (let octave = -3; octave <= 3; octave++) {
							...
							if (Math.abs(freq - refFreq) < freqPerBin / 2) {
								this.chroma[n] += ensureFinite(magnitudes[i + j] * magnitudes[i + j] * (spectralProfile.profile === "vocal" ? 1.1 : 1));
							}
						}
					}
				}
			...
			if (sum > 0) this.chroma.set(this.chroma.map(v => ensureFinite(v / sum)));
		} catch (error) {
			handleError("Chromagram computation failed", error, {
				fftSize: this.fftSize
			});
			throw error;
		}
	...
	computeDynamicRange(timeData) {
		let min = Infinity,
			max = -Infinity;
		for (let i = 0; i < timeData.length; i += 16) { // Vectorized x16
			for (let j = 0; j < 16 && i + j < timeData.length; j++) {
				min = Math.min(min, timeData[i + j]);
				max = Math.max(max, timeData[i + j]);
			}
		}
		return ensureFinite(20 * Math.log10((max - min) / (Math.abs(min) || 1)));
	}
	...
	dct(data) {
		...
		try {
			for (let k = 0; k < n; k++) {
				...
				result[k] = ensureFinite(sum * (k === 0 ? 1 / Math.sqrt(n) : Math.sqrt(2 / n)));
			}
			return result;
		} catch (error) {
			handleError("DCT computation failed", error, {
				dataLength: n
			});
			throw error;
		}
	...
	computeSpectralFlatness(magnitudes) {
		let sum = 0,
			logSum = 0,
			count = 0;
		for (let i = 0; i < magnitudes.length; i += 16) { // Vectorized x16
			for (let j = 0; j < 16 && i + j < magnitudes.length; j++) {
				if (magnitudes[i + j] > 0) {
					sum += magnitudes[i + j];
					logSum += Math.log(magnitudes[i + j]);
					count++;
				}
			}
		}
		...
		return ensureFinite(geoMean / (mean || 1));
	}

	computeZeroCrossingRate(timeData) {
		let crossings = 0;
		for (let i = 1; i < timeData.length; i += 16) { // Vectorized x16
			for (let j = 0; j < 16 && i + j < timeData.length; j++) {
				if (timeData[i + j] * timeData[i + j - 1] < 0) crossings++;
			}
		}
		return ensureFinite(crossings / timeData.length);
	}
	...
	computeSpectralCentroid(magnitudes) {
		...
		for (let i = 0; i < magnitudes.length; i += 16) { // Vectorized x16
			for (let j = 0; j < 16 && i + j < magnitudes.length; j++) {
				sumMag += magnitudes[i + j];
				weightedSum += (i + j) * magnitudes[i + j];
			}
		}
		return sumMag > 0 ? ensureFinite(weightedSum / sumMag) : 0;
	}

	smoothProfile(profile, instruments, spectralProfile = {}) {
		...
		if (["EDM", "DrumAndBass", "Hip-Hop"].includes(genre) || spectralProfile.profile === "bassHeavy" || spectralProfile.profile === "rockMetal") {
			alpha = Math.max(0.4, 0.65 - transientEnergy * 0.2);
		} else if (["Classical", "Jazz", "Bolero"].includes(genre) || spectralProfile.profile === "warm" || spectralProfile.profile === "proNatural") {
			alpha = Math.min(0.8, 0.65 + (1 - transientEnergy) * 0.2);
		}
		if (profile.vocalPresence > 0.55 || spectralProfile.profile === "vocal" || spectralProfile.profile === "karaokeDynamic" || Object.values(instruments).some(conf => conf > 0.5)) {
		...
		try {
			for (const key in profile) {
				if (Array.isArray(profile[key])) {
					smoothed[key] = profile[key].map((v, i) => ensureFinite(alpha * v + (1 - alpha) * (prev[key]?.[i] || v)));
				} else if (typeof profile[key] === "object") {
					smoothed[key] = Object.fromEntries(
						Object.entries(profile[key]).map(([k, v]) => [k, ensureFinite(alpha * v + (1 - alpha) * (prev[key]?.[k] || v))])
					);
				} else {
					smoothed[key] = ensureFinite(alpha * profile[key] + (1 - alpha) * (prev[key] || profile[key]));
				}
			}
			return smoothed;
		} catch (error) {
			handleError("Profile smoothing failed", error, {
				profile
			});
			throw error;
		}
	...
	sigmoid(x) {
		return ensureFinite(1 / (1 + Math.exp(-x)));
	}

	classifyGenre(profile, spectralProfile = {}) {
		const {
			transientEnergy,
			vocalPresence,
			spectralFlatness,
		...
		if (spectralProfile.profile === "bassHeavy" || (transientEnergy > 0.7 && bass > 0.6)) return "EDM";
		if (spectralProfile.profile === "vocal" || spectralProfile.profile === "karaokeDynamic" || (vocalPresence > 0.6 && spectralFlatness < 0.4)) return "Pop";
		if (spectralProfile.profile === "warm" || spectralProfile.profile === "proNatural" || (high > 0.5 && spectralFlatness > 0.6)) return "Classical";
		if (spectralProfile.profile === "rockMetal" || (transientEnergy > 0.6 && bass > 0.5)) return "Rock";
		if (spectralProfile.profile === "bright" || spectralProfile.profile === "smartStudio") return "Pop";
		return "Pop";
	}
	...
	dispose() {
		try {
			this.melFilterBanks.forEach(bank => this.memoryManager.free(bank));
			this.memoryManager.free(this.chroma);
			this.memoryManager.free(this.lastMagnitudes);
			this.memoryManager.free(this.junBuffer);
			Object.values(this.instrumentSignatures).forEach(({
				template
			}) => this.memoryManager.free(template));
			this.transformer.layers.forEach(layer => {
			...
			this.fft.dispose();
			console.debug(`SpectralAnalyzer disposed for fftSize ${this.fftSize}`);
		} catch (error) {
			handleError("Failed to dispose SpectralAnalyzer", error, {
				fftSize: this.fftSize
			});
		}
	}
...
// Test FFT
async function testFFT(junFileData, memoryManager, webGPUDevice, spectralProfile = {}, noiseInfo = {
	type: "white",
	confidence: 0.5
}) {
	if (!memoryManager) throw new Error("MemoryManager is required");
	...
	try {
		...
		const {
			magnitudes
		} = getMagnitudeAndPhase(sineFftData, 1024, spectralProfile, noiseInfo);
		...
		const {
			magnitudes: noisyMagnitudes
		} = getMagnitudeAndPhase(noisyFftData, 1024, spectralProfile, noiseInfo);
		...
		if (junFileData) {
			...
			const {
				magnitudes: junMagnitudes,
				phases
			} = getMagnitudeAndPhase(junFftData, 1024, spectralProfile, noiseInfo);
			const {
				frequencies
			} = timeFrequencyReassignment(junMagnitudes, phases, 1024, sampleRate, 256, noiseInfo, spectralProfile);
...
class SpectralSubtraction {
	 ...
	 */
	async process(magnitudes, noiseLevel, fftSize, sampleRate, spectralHistory, noiseInfo = {
		type: "white",
		confidence: 0.5
	}, spectralProfile = {}, onsetPrediction = 0.5) {
		const startTime = performance.now();
		...
		if (this.cache.has(cacheKey)) {
			...
			if (Date.now() - cached.timestamp < this.cacheTTL) {
				this.performanceMetrics.cacheHits++;
				this.notify("spectral_subtraction_completed", {
					cacheHit: true
				});
				return cached.result;
			}
...
class NoiseTypeDetector {
	...
	async detect(magnitudes, sampleRate, fftSize, spectralProfile, onsetPrediction = 0.5, spectralHistory = []) {
		...
		if (this.cache.has(cacheKey)) {
			...
			if (Date.now() - cached.timestamp < this.cacheTTL) {
				this.performanceMetrics.cacheHits++;
				this.notify("noise_detection_completed", {
					cacheHit: true
				});
				return cached.result;
			}
		...
		const bindGroup = this.webGPUDevice.createBindGroup({
			...
			entries: [{
				...
				},
				{
					binding: 1,
					resource: {
						buffer: weightsTanhBuffer
					}
				},
				{
					binding: 2,
					resource: {
						buffer: weightsSigmoidBuffer
					}
				},
				{
					binding: 3,
	...
	computeWaveNetCPU(input, layer) {
		...
		for (let f = 0; f < this.filters; f += 4) {
			for (let k = 0; k < 4 && f + k < this.filters; k++) {
				const idx = f + k;
				let tanhSum = biases[layer][idx],
					sigmoidSum = biases[layer][idx];
...
class ArtifactSuppressor {
	...
	async suppress(signal, spectralProfile, noiseInfo = {
		type: "white",
		confidence: 0.5
	}, rms = 0.1, junMetadata = {}) {
		...
		const noiseParams = {
			white: {
				baseFactor: 0.8,
				freqWeight: [1.0, 1.0, 1.0, 1.0],
				transientThreshold: 0.1
			},
			pink: {
				baseFactor: 0.75,
				freqWeight: [1.2, 1.0, 0.8, 0.6],
				transientThreshold: 0.12
			},
			brown: {
				baseFactor: 0.7,
				freqWeight: [1.5, 1.2, 0.7, 0.5],
				transientThreshold: 0.15
			},
			lowFreq: {
				baseFactor: 0.65,
				freqWeight: [1.8, 1.0, 0.5, 0.3],
				transientThreshold: 0.2
			},
			midFreq: {
				baseFactor: 0.7,
				freqWeight: [0.5, 1.5, 1.0, 0.5],
				transientThreshold: 0.18
			},
			highFreq: {
				baseFactor: 0.85,
				freqWeight: [0.3, 0.5, 1.0, 1.5],
				transientThreshold: 0.08
			},
			transient: {
				baseFactor: 0.9,
				freqWeight: [0.5, 0.7, 1.0, 1.2],
				transientThreshold: 0.05
			}
		};
...
class InstrumentSeparator {
	...
	async separate(magnitudes, fftSize, sampleRate, spectralProfile, noiseInfo = {
		type: "white",
		confidence: 0.5
	}, junMetadata = {}) {
		...
		const separated = {};
		const instrumentNames = Object.keys(spectralProfile.instruments || {
			"instrument1": 0,
			"instrument2": 1,
			"instrument3": 2,
			"instrument4": 3
		}).slice(0, this.numComponents);
		for (let k = 0; k < this.numComponents; k++) {
		...
		for (let i = 0; i < signal.length; i += fftSize) {
			...
			try {
				...
				({
					magnitudes,
					phases
				} = getMagnitudeAndPhase(fftData, fftSize));
			} catch (error) {
...
function getHRTFStats(spectralProfile, userParams) {
	return {
		azimuth: userParams.azimuth ?? spectralProfile.instruments?.guitar ? 30 : spectralProfile.instruments?.piano ? 15 : spectralProfile.instruments?.violin ? 45 : 0,
...
class AudioProcessor {
	...
	process(input, userParams = {}) {
		...
		this.harmonicBoost = userParams.harmonicBoost || {
			guitar: 1.3,
			piano: 1.2
		};
			...
			while (framePos < signal.length) {
				...
				try {
					({
						magnitudes,
						phases
					} = getMagnitudeAndPhase(fftData, this.fftSize));
				} catch (error) {
	...
	function estimateKey(spectralProfile, noiseInfo) {
		...
		return {
			key: bestKey,
			confidence: maxScore / (chroma.reduce((sum, v) => sum + v, 0) + 1e-10),
			isMajor
		};
	}
	...
	self.onmessage = async function(e) {
		...
		if (command === "init") {
			...
			try {
				...
				const {
					magnitudes
				} = getMagnitudeAndPhase(fftData, effectiveFftSize);
	...
	function analyzeSpectral(input, fftSize, memoryManager, spectralProfile) {
		...
		if (cachedResult && cachedResult.timestamp > Date.now() - 1000) {
			console.debug("Using cached spectral result", {
				cacheKey
			});
		...
		try {
			...
			const qualityPrediction = predictQuality({
				...
				spectralProfile.noiseLevel || {
					confidence: 0
				}, {
					treble: transientEnergy,
					harmonicRatio
				}
			);
	...
	function analyzeTimeDomain(timeData, sampleRate, memoryManager, detectNoiseType, spectralProfile) {
		...
		// Check cache
		if (cachedResult && cachedResult.timestamp > Date.now() - 1000) {
			console.debug("Using cached time-domain result", {
				cacheKey
			});
			return cachedResult.data;
		}
		...
		try {
			...
			const result = {
				...
				transientEnergy,
				spectralFlatness: spectralProfile.spectralFlatness || 0.5,
				instruments: spectralProfile.instruments || {
					guitar: 0.3,
					piano: 0.2,
					drums: 0.5,
					violin: 0.1
			...
			handleError("Time-domain analysis failed:", error, {
				sampleRate
			});
			self.postMessage({
	...
	function detectPolyphonicPitches(magnitudes, fftSize, sampleRate, spectralProfile, memoryManager, noiseInfo, pitchInfo, suppressArtifacts) {
		...
		try {
			...
			const {
				output: cleanedMagnitudes
			} = suppressArtifacts(magnitudes, spectralProfile, noiseInfo, memoryManager, pitchInfo.rms || 0.1);
			...
			const result = {
				...
				metadata: {
					numComponents: groupedPitches.length,
					iterations: 50
				}
			};
	...
	/**
	 * Audio analyzer with advanced FFT, NMF, transient detection, and noise suppression,
	 * optimized for HiFi audio, synchronized with jungle.js and OptimizedFFT.js.
	 */
class AudioAnalyzer {
    constructor(sampleRate, fftSize = 2048, devicePerf = "medium", memoryManager) {
        if (!Number.isInteger(fftSize) || fftSize < 16 || fftSize > 262144 || (fftSize & (fftSize - 1)) !== 0) {
            throw new Error(`Invalid FFT size: ${fftSize} (must be a power of 2, 16 <= size <= 262144)`);
        }
        if (!memoryManager) throw new Error("MemoryManager is required");
        ...
        this.analyzers = {};
        this.fft = new OptimizedFFT(fftSize, memoryManager, true, null, { devicePerf });

        // Hằng số từ cla3aProcessor.js
        this.TWO_PI = 2 * Math.PI;
        this.SMALL_VALUE = 1e-10;
        this.MAX_SIGNAL_VALUE = 1.0;
        this.MIN_SIGNAL_VALUE = -1.0;

        // Bảng tra cứu từ cla3aProcessor.js
        this.LOG2_LOOKUP = new Float32Array(1000);
        for (let i = 0; i < 1000; i++) {
            this.LOG2_LOOKUP[i] = Math.log2((i + 1) / 1000);
        }
        this.TANH_LUT = new Float32Array(1000);
        for (let i = 0; i < 1000; i++) {
            const x = (i / 999) * 10 - 5;
            this.TANH_LUT[i] = Math.tanh(x);
        }

        // Compressor parameters từ cla3aProcessor.js
        this.compressorParams = {
            ...
            attackMode: "Fast",
            transientAmount: 1.0,
            cochlearCompression: 0.5,
            entropyColorationFactor: 0.2,
            breathingDepth: 0.1,
            waveformShapeFactor: 0.5,
            artifactAmount: 0.1,
            enableSpectralCompression: true,
            enableCochlearModel: true,
            enableControlledArtifacts: false
        };

        // QuantumHarmonicProcessor parameters
        this.qhp = {
            harmonicBuffer: null,
            phaseBuffer: null,
            transientSculptBuffer: null,
            parameters: {
                harmonicBoost: 1.0,
                transientSculpt: 1.0,
                phaseCoherenceFactor: 0.8,
                emotionalModeFactor: 1.0
            }
        };
        ...
        // Cập nhật HiFi AT2030 parameters: Thêm listenerProfile, timbreProfile
        this.at2030Params = {
            order: (devicePerf === "high" ? 5 : devicePerf === "medium" ? 4 : 3),
            sigma: Math.max(0.1, Math.min(0.5, 0.3)),
            vocalPresence: Math.max(0.7, Math.min(1.3, 1.0)),
            bassLevel: Math.max(0.3, Math.min(0.9, 0.5)),
            midGain: Math.max(0.5, Math.min(1.0, 0.75)),
            trebleQ: Math.max(0.5, Math.min(1.0, 0.7)),
            sculptFactor: Math.max(0.8, Math.min(1.5, 1.0)),
            emotionalVector: Math.max(0.8, Math.min(1.2, 1.0)),
            formantScale: Math.max(0.8, Math.min(1.2, 1.0)),
            phaseLockFactor: Math.max(0.8, Math.min(1.2, 1.0)),
            pitchShift: 0,
            listenerProfile: "standard", // Thêm: standard, audiophile, casual
            timbreProfile: "neutral" // Thêm: warm, bright, vocal, etc.
        };
        this.at2030Buffer = null;
    }

    fastExp(x) {
        if (x < -10) return 0;
        if (x > 10) return Math.exp(10);
        const x2 = x * x;
        return 1 + x + x2 / 2 + x2 * x / 6;
    }
    ...
    initialize() {
        try {
            ...
            this.applyCompressorPreset(this.compressorParams);
            self.postMessage({
                type: "analyzerInitialized",
                status: "success",
                fftSize: this.fftSize
            });
        } catch (error) {
            handleError("Failed to initialize analyzers", error, { fftSize: this.fftSize });
            self.postMessage({
                type: "analyzerError",
    ...
    selectWindowType(spectralProfile = {}) {
        const { transientEnergy = 0.5, vocalPresence = 0.5, currentGenre = "Pop", spectralFlatness = 0.5, profile } = spectralProfile;
        if (profile === "bassHeavy" || transientEnergy > 0.8 || ["DrumAndBass", "Techno", "EDM"].includes(currentGenre)) {
            return "kaiser";
        }
        if (profile === "vocal" || profile === "karaokeDynamic" || vocalPresence > 0.7 || currentGenre === "Karaoke") {
            return "hamming";
        }
        if (profile === "warm" || profile === "proNatural" || currentGenre === "Classical" || spectralFlatness > 0.7) {
            return "blackman-harris";
        }
        if (profile === "rockMetal") {
            return "nuttall";
        }
    ...
    initializeFFT() {
        this.analyzers.fft = {
            ...
            initWindow: (spectralProfile = {}) => {
                const windowType = this.selectWindowType(spectralProfile);
                const tScale = this.TWO_PI / (this.fftSize - 1);
                for (let i = 0; i < this.fftSize; i += 16) {
                    for (let j = 0; j < 16 && i + j < this.fftSize; j++) {
                        const t = tScale * (i + j);
                        if (windowType === "blackman-harris") {
                            const a0 = 0.35875, a1 = 0.48829, a2 = 0.14128, a3 = 0.01168;
                            this.analyzers.fft.window[i + j] = ensureFinite(a0 - a1 * Math.cos(t) + a2 * Math.cos(2 * t) - a3 * Math.cos(3 * t));
                        } else if (windowType === "kaiser") {
                            const beta = spectralProfile.transientEnergy > 0.8 ? 12.0 : spectralProfile.transientEnergy > 0.6 ? 10.0 : 8.0;
                            const bessel = (x) => {
                            ...
                            this.analyzers.fft.window[i + j] = ensureFinite(bessel(beta * Math.sqrt(1 - (2 * (i + j) / (this.fftSize - 1) - 1) ** 2)) / bessel(beta));
                        } else if (windowType === "nuttall") {
                            const a0 = 0.355768, a1 = 0.487396, a2 = 0.144232, a3 = 0.012604;
                            this.analyzers.fft.window[i + j] = ensureFinite(a0 - a1 * Math.cos(t) + a2 * Math.cos(2 * t) - a3 * Math.cos(3 * t));
                        } else if (windowType === "blackman") {
                            this.analyzers.fft.window[i + j] = ensureFinite(0.42 - 0.5 * Math.cos(t) + 0.08 * Math.cos(2 * t));
                        } else if (windowType === "hann") {
                            this.analyzers.fft.window[i + j] = ensureFinite(0.5 * (1 - Math.cos(t)));
                        } else {
                            this.analyzers.fft.window[i + j] = ensureFinite(0.54 - 0.46 * Math.cos(t));
                        }
                    }
                }
                console.debug(`FFT window initialized: ${windowType}, size: ${this.fftSize}`);
            },
            process: async (input, spectralProfile = {}, noiseInfo = { type: "white", confidence: 0.5 }) => {
                if (input.length < this.fftSize) {
                    handleError("Input too short for FFT", new Error(`Expected ${this.fftSize}, got ${input.length}`));
                    throw new Error(`Input too short for FFT: ${input.length}`);
                }
                for (let i = 0; i < this.fftSize; i += 16) {
                    for (let j = 0; j < 16 && i + j < this.fftSize; j++) {
                        this.analyzers.fft.timeData[i + j] = ensureFinite(input[i + j] * this.analyzers.fft.window[i + j]);
                    }
                }
                const fftResult = await this.fft.fft(this.analyzers.fft.timeData, noiseInfo, suppressArtifacts);
                this.analyzers.fft.index = (this.analyzers.fft.index + 1) % this.fftSize;
                for (let i = 0; i < this.fftSize; i++) {
                    const idx = (this.analyzers.fft.index - i + this.fftSize) % this.fftSize;
                    this.analyzers.fft.real[i] = this.analyzers.fft.timeData[idx] * this.analyzers.fft.window[i];
                    this.analyzers.fft.imag[i] = 0;
                }
                let n = this.fftSize;
                for (let i = 0; i < n; i++) {
                    let j = 0;
                    ...
                    if (j > i) {
                        [this.analyzers.fft.real[i], this.analyzers.fft.real[j]] = [this.analyzers.fft.real[j], this.analyzers.fft.real[i]];
                        [this.analyzers.fft.imag[i], this.analyzers.fft.imag[j]] = [this.analyzers.fft.imag[j], this.analyzers.fft.imag[i]];
                    }
                }
                for (let step = 2; step <= n; step *= 2) {
                    const half = step / 2;
                    const angleStep = -this.TWO_PI / step;
                    for (let k = 0; k < n; k += step) {
                        for (let i = 0; i < half; i++) {
                            const angle = angleStep * i;
                            const wReal = Math.cos(angle);
                            const wImag = Math.sin(angle);
                            const tReal = wReal * this.analyzers.fft.real[k + i + half] - wImag * this.analyzers.fft.imag[k + i + half];
                            const tImag = wReal * this.analyzers.fft.imag[k + i + half] + wImag * this.analyzers.fft.real[k + i + half];
                            this.analyzers.fft.real[k + i + half] = this.analyzers.fft.real[k + i] - tReal;
                            this.analyzers.fft.imag[k + i + half] = this.analyzers.fft.imag[k + i] - tImag;
                            this.analyzers.fft.real[k + i] += tReal;
                            this.analyzers.fft.imag[k + i] += tImag;
                        }
                    }
                }
                for (let i = 0; i < this.fftSize / 2; i++) {
                    this.analyzers.fft.magnitudes[i] = ensureFinite(Math.sqrt(this.analyzers.fft.real[i] * this.analyzers.fft.real[i] + this.analyzers.fft.imag[i] * this.analyzers.fft.imag[i]));
                }
                if (spectralProfile.profile === "bassHeavy" || spectralProfile.transientEnergy > 0.8) {
                    for (let i = 0; i < 32; i++) {
                        fftResult[i * 2] *= 1.1;
                        this.analyzers.fft.magnitudes[i] *= 1.1;
                    }
                }
                if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
                    for (let i = 32; i < 128; i++) {
                ...
                let sumFreq = 0, sumMag = 0;
                for (let i = 0; i < this.analyzers.fft.magnitudes.length; i++) {
                    const freq = (i * this.sampleRate) / this.fftSize;
                    sumFreq += freq * this.analyzers.fft.magnitudes[i];
                    sumMag += this.analyzers.fft.magnitudes[i];
                }
                this.analyzers.fft.spectralCentroid = sumMag > 0 ? sumFreq / sumMag : 0;
                let energySum = 0;
                for (let i = 0; i < this.analyzers.fft.magnitudes.length; i++) {
                    energySum += this.analyzers.fft.magnitudes[i] * this.analyzers.fft.magnitudes[i];
                }
                const bpmRangeMin = 60, bpmRangeMax = 180;
                const freqMin = (bpmRangeMin / 60) * (this.fftSize / this.sampleRate);
                const freqMax = (bpmRangeMax / 60) * (this.fftSize / this.sampleRate);
                let maxEnergy = 0, bestBpm = bpmRangeMin;
                for (let i = Math.floor(freqMin); i < Math.min(freqMax, this.analyzers.fft.magnitudes.length); i++) {
                    const energy = this.analyzers.fft.magnitudes[i] * this.analyzers.fft.magnitudes[i];
                    if (energy > maxEnergy) {
                        maxEnergy = energy;
                        bestBpm = (i * 60 * this.sampleRate) / this.fftSize;
                    }
                }
                this.analyzers.fft.tempo = ensureFinite(bestBpm);
                return this.analyzers.fft.magnitudes;
            }
        };
    ...
    initializeNMF() {
        this.analyzers.nmf = {
            components: this.devicePerf === "low" ? 4 : this.devicePerf === "medium" ? 6 : 8,
            iterations: this.devicePerf === "low" ? 50 : this.devicePerf === "medium" ? 75 : 100,
                ...
                try {
                    ...
                    if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
                        pitches.forEach(pitch => {
                            if (pitch.frequency >= 200 && pitch.frequency <= 2000) {
                                pitch.confidence *= 1.1;
                            }
                        });
                    }
                    return pitches;
                } catch (error) {
                    handleError("NMF pitch detection failed", error, { fftSize: this.fftSize });
                    return [];
                }
            }
        };
    ...
    initializeTransientDetector() {
        this.analyzers.transient = {
            threshold: this.devicePerf === "low" ? 0.2 : 0.15,
            history: this.memoryManager.allocate(this.fftSize),
            lastEnergy: 0,
            process: (input, spectralProfile) => {
                let energy = 0;
                for (let i = 0; i < input.length && i < this.fftSize; i += 16) {
                    for (let j = 0; j < 16 && i + j < input.length && i + j < this.fftSize; j++) {
                        energy += input[i + j] * input[i + j];
                    }
                }
                energy = ensureFinite(Math.sqrt(energy / Math.min(input.length, this.fftSize)));
                const delta = ensureFinite(Math.abs(energy - this.analyzers.transient.lastEnergy));
                this.analyzers.transient.lastEnergy = energy;
                const threshold = this.analyzers.transient.threshold * (1 + (spectralProfile.transientEnergy || 0.5) * 0.5);
                if (spectralProfile.profile === "bassHeavy" || spectralProfile.profile === "rockMetal") {
                    return delta > threshold * 0.8 ? energy : 0;
                }
                return delta > threshold ? energy : 0;
            }
        };
    }
    ...
    initializeNoiseSuppressor() {
        this.analyzers.noiseSuppressor = {
            noiseProfile: null,
            gainReduction: this.devicePerf === "low" ? 0.7 : 0.5,
            updateNoiseProfile: (noiseData) => {
                try {
                    this.analyzers.noiseSuppressor.noiseProfile = this.memoryManager.allocate(noiseData.length);
                    for (let i = 0; i < noiseData.length && i < this.fftSize; i += 16) {
                        for (let j = 0; j < 16 && i + j < noiseData.length && i + j < this.fftSize; j++) {
                            this.analyzers.noiseSuppressor.noiseProfile[i + j] = ensureFinite(noiseData[i + j]);
                        }
                    }
                } catch (error) {
                    handleError("Failed to update noise profile", error, { fftSize: this.fftSize });
                    this.memoryManager.free(this.analyzers.noiseSuppressor.noiseProfile);
                    this.analyzers.noiseSuppressor.noiseProfile = null;
                }
            },
            process: (input, spectralProfile, noiseInfo) => {
                if (!this.analyzers.noiseSuppressor.noiseProfile) return input;
                const output = suppressArtifacts(
                    input,
                    spectralProfile,
                    noiseInfo,
                    this.memoryManager,
                ...
                if (spectralProfile.profile === "bassHeavy" || spectralProfile.transientEnergy > 0.8) {
                    for (let i = 0; i < output.length && i < this.fftSize; i++) {
                        if (i < 32) output[i] = ensureFinite(output[i] * 1.1);
                    }
                }
                if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
                    for (let i = 0; i < output.length && i < this.fftSize; i++) {
                        if (i >= 32 && i < 128) output[i] = ensureFinite(output[i] * 1.1);
                    }
                }
                if (this.compressorParams.enableControlledArtifacts && (spectralProfile.profile === "rockMetal" || spectralProfile.profile === "smartStudio")) {
                    for (let i = 0; i < output.length && i < this.fftSize; i++) {
                        const noise = this.artifactGenerator.noiseType === "white" ? (Math.random() - 0.5) * 0.1 : 0;
                        output[i] = ensureFinite(output[i] + noise * this.compressorParams.artifactAmount);
                    }
                }
                return output;
            }
    ...
    initializeQHP() {
        try {
            this.qhp.harmonicBuffer = this.memoryManager.allocate(this.fftSize);
            this.qhp.phaseBuffer = this.memoryManager.allocate(this.fftSize * 2);
            this.qhp.transientSculptBuffer = this.memoryManager.allocate(this.fftSize);
            this.at2030Buffer = this.memoryManager.allocate(this.fftSize * 2);
            this.at2030Buffer.fill(0);
            console.debug("QuantumHarmonicProcessor initialized with AT2030 buffer");
        } catch (error) {
            handleError("Failed to initialize QHP", error, { fftSize: this.fftSize });
        }
    }
    ...
    applyCompressorPreset(preset) {
        try {
            for (const [key, value] of Object.entries(preset)) {
                if (this.compressorParams[key] !== undefined) {
                    this.compressorParams[key] = ensureFinite(value, this.compressorParams[key]);
                } else {
                    handleError("Invalid compressor preset key", new Error(`Key ${key} not found`), { preset });
                }
            }
        } catch (error) {
            handleError("Failed to apply compressor preset", error, { preset });
        }
    }

    getCompressorState() {
        return { ...this.compressorParams, emotionalMode: this.contentAnalyzer.emotionalMode };
    }

    compressCLA3A(input, spectralProfile) {
        try {
            const output = new Float32Array(input.length);
            if (!this.rmsBuffer) {
            ...
            let rmsSum = 0, index = 0;
            const windowSize = this.fftSize;
            const { threshold, ratio, makeupGain, transientAmount, cochlearCompression, entropyColorationFactor, breathingDepth } = this.compressorParams;
            let sculptFactor = this.at2030Params.sculptFactor;
            let emotionalVector = this.at2030Params.emotionalVector;
            let transientEnergy = 0;
            for (let i = 0; i < input.length; i++) {
                transientEnergy += Math.abs(input[i]);
            }
            transientEnergy = ensureFinite(transientEnergy / input.length);
            switch (spectralProfile.profile) {
                case "warm": sculptFactor = 1.1; emotionalVector = 0.9; break;
                case "bright": sculptFactor = 1.2; emotionalVector = 1.0; break;
                case "bassHeavy": sculptFactor = 1.3; emotionalVector = 1.05; break;
                case "vocal": sculptFactor = 1.0; emotionalVector = 1.0; break;
                case "proNatural": sculptFactor = 1.05; emotionalVector = 1.0; break;
                case "karaokeDynamic": sculptFactor = 1.4; emotionalVector = 1.1; break;
                case "rockMetal": sculptFactor = 1.5; emotionalVector = 1.1; break;
                case "smartStudio": sculptFactor = 1.25; emotionalVector = 1.05; break;
            }
            let cpuLoad = (this.devicePerf === "low" ? 0.9 : this.devicePerf === "medium" ? 0.6 : 0.3);
            let DeviceAdaptFactor = Math.max(0.7, Math.min(1.0, 1.0 - (cpuLoad * 0.3)));
            let TransientSculpt = transientEnergy * sculptFactor * DeviceAdaptFactor;
            let spectralFlux = 0;
            for (let i = 1; i < this.analyzers.fft.magnitudes.length; i++) {
                spectralFlux += Math.abs(this.analyzers.fft.magnitudes[i] - this.analyzers.fft.magnitudes[i - 1]);
            }
            spectralFlux = ensureFinite(spectralFlux / (this.analyzers.fft.magnitudes.length - 1));
            let softmax = (val) => Math.exp(val) / (Math.exp(0.9) + Math.exp(1.0) + Math.exp(1.1));
            if (this.contentAnalyzer.emotionalMode === "calm") emotionalVector = softmax(0.9 * this.contentAnalyzer.energy * this.contentAnalyzer.dynamics * spectralFlux);
            else if (this.contentAnalyzer.emotionalMode === "aggressive") emotionalVector = softmax(1.1 * this.contentAnalyzer.energy * this.contentAnalyzer.dynamics * spectralFlux);
            else emotionalVector = softmax(1.0 * this.contentAnalyzer.energy * this.contentAnalyzer.dynamics * spectralFlux);
            for (let i = 0; i < input.length; i++) {
                rmsSum += input[i] * input[i];
                this.rmsBuffer[index] = input[i] * input[i];
                rmsSum -= this.rmsBuffer[(index + 1) % windowSize] || 0;
                index = (index + 1) % windowSize;
                let rms = ensureFinite(Math.sqrt(rmsSum / windowSize), this.SMALL_VALUE);
                let gainReduction = rms > threshold ? (rms - threshold) * (1 - 1 / ratio) : 0;
                gainReduction = gainReduction / TransientSculpt * emotionalVector * DeviceAdaptFactor;
                gainReduction = Math.min(Math.max(gainReduction, 0.8), 1.2);
                output[i] = ensureFinite(input[i] * (1 - gainReduction) * makeupGain, this.MAX_SIGNAL_VALUE);
                if (spectralProfile.profile === "bassHeavy" || spectralProfile.transientEnergy > 0.8) {
                    this.transientBuffer[i % windowSize] = ensureFinite(input[i] * transientAmount);
                    output[i] = ensureFinite(output[i] + this.transientBuffer[i % windowSize] * 0.2);
                }
                if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
                    output[i] = ensureFinite(this.TANH_LUT[Math.floor((output[i] + 5) * 99.9)] * cochlearCompression + output[i] * (1 - cochlearCompression));
                }
                if (this.compressorParams.enableControlledArtifacts) {
                    const entropy = (Math.random() - 0.5) * entropyColorationFactor;
                    output[i] = ensureFinite(output[i] + entropy);
                }
                if (breathingDepth > 0 && this.analyzers.fft.tempo) {
                    const phase = (i / this.sampleRate) * this.analyzers.fft.tempo * this.TWO_PI;
                    output[i] = ensureFinite(output[i] * (1 + breathingDepth * Math.sin(phase)));
                }
            }
            return output;
        } catch (error) {
            handleError("CLA-3A compression failed", error, { fftSize: this.fftSize, inputLength: input.length });
            return input;
        }
    }

    analyzeContent(input, spectralProfile) {
        try {
            let energy = 0, dynamics = 0;
            for (let i = 0; i < input.length && i < this.fftSize; i++) {
                energy += input[i] * input[i];
                dynamics += Math.abs(input[i] - (input[i - 1] || 0));
            }
            energy = ensureFinite(Math.sqrt(energy / Math.min(input.length, this.fftSize)));
            dynamics = ensureFinite(dynamics / Math.min(input.length, this.fftSize));
            this.contentAnalyzer.energy = energy;
            this.contentAnalyzer.dynamics = dynamics;
            this.classifyEmotionalContent(spectralProfile);
        } catch (error) {
            handleError("Content analysis failed", error, { fftSize: this.fftSize });
        }
    }

    classifyEmotionalContent(spectralProfile) {
        try {
            const { energy, dynamics } = this.contentAnalyzer;
            if (spectralProfile.profile === "warm" || spectralProfile.spectralFlatness > 0.7) {
                this.contentAnalyzer.emotionalMode = energy < 0.3 && dynamics < 0.2 ? "calm" : "neutral";
                this.at2030Params.emotionalVector = energy < 0.3 && dynamics < 0.2 ? 0.9 : 1.0;
            } else if (spectralProfile.profile === "rockMetal" || spectralProfile.transientEnergy > 0.8) {
                this.contentAnalyzer.emotionalMode = energy > 0.5 && dynamics > 0.4 ? "aggressive" : "neutral";
                this.at2030Params.emotionalVector = energy > 0.5 && dynamics > 0.4 ? 1.1 : 1.0;
            } else {
                this.contentAnalyzer.emotionalMode = "neutral";
                this.at2030Params.emotionalVector = 1.0;
            }
            // Cập nhật timbreProfile dựa trên profile
            this.at2030Params.timbreProfile = spectralProfile.profile || "neutral";
        } catch (error) {
            handleError("Emotional content classification failed", error, { fftSize: this.fftSize });
            this.contentAnalyzer.emotionalMode = "neutral";
            this.at2030Params.emotionalVector = 1.0;
            this.at2030Params.timbreProfile = "neutral";
        }
    }

    // Thêm hàm mới: Tính SpectralAttention
    computeSpectralAttention(freqData, spectralProfile) {
        try {
            const spectralAttention = new Float32Array(this.fftSize / 2);
            let spectralFlux = 0;
            for (let i = 1; i < freqData.length; i++) {
                spectralFlux += Math.abs(freqData[i] - freqData[i - 1]);
            }
            spectralFlux = ensureFinite(spectralFlux / (freqData.length - 1));
            let sumExp = 0;
            for (let i = 0; i < this.fftSize / 2; i++) {
                const energy = freqData[i] * freqData[i];
                spectralAttention[i] = this.fastExp(energy * spectralFlux);
                sumExp += spectralAttention[i];
            }
            for (let i = 0; i < this.fftSize / 2; i++) {
                spectralAttention[i] = ensureFinite(spectralAttention[i] / (sumExp + this.SMALL_VALUE), 1.0);
            }
            // Tăng attention cho vocal và transient
            if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
                for (let i = Math.floor(200 * this.fftSize / this.sampleRate); i < Math.floor(2000 * this.fftSize / this.sampleRate); i++) {
                    spectralAttention[i] *= 1.2;
                }
            }
            if (spectralProfile.profile === "rockMetal" || spectralProfile.transientEnergy > 0.8) {
                for (let i = 0; i < 32; i++) {
            ...
            handleError("SpectralAttention computation failed", error, { fftSize: this.fftSize });
            return new Float32Array(this.fftSize / 2).fill(1.0);
        }
    }

    // Thêm hàm mới: Tính PsychoacousticWeight
    computePsychoacousticWeight(spectralProfile) {
        try {
            const psychoacousticWeight = new Float32Array(this.fftSize / 2);
            const freqStep = this.sampleRate / this.fftSize;
            // Fletcher-Munson curve đơn giản hóa
            ...
            for (let i = 0; i < this.fftSize / 2; i++) {
                ...
                const maskingThreshold = Math.pow(10, -60 / 20) * fletcherMunson(freq); // -60 dB SPL
                let perceptualSensitivity = this.at2030Params.listenerProfile === "audiophile" ? 1.1 : this.at2030Params.listenerProfile === "casual" ? 0.9 : 1.0;
                psychoacousticWeight[i] = ensureFinite(maskingThreshold * perceptualSensitivity, 1.0);
            }
            return psychoacousticWeight;
        } catch (error) {
            handleError("PsychoacousticWeight computation failed", error, { fftSize: this.fftSize });
            return new Float32Array(this.fftSize / 2).fill(1.0);
        }
    }

    // Thêm hàm mới: Tính EmotionTimbreMap
    computeEmotionTimbreMap(spectralProfile) {
        try {
            const timbreCurve = new Float32Array(this.fftSize / 2);
            const freqStep = this.sampleRate / this.fftSize;
            // Cubic spline đơn giản hóa cho timbreProfile
            const splinePoints = {
            ...
            const profilePoints = splinePoints[spectralProfile.profile] || splinePoints.neutral;
            for (let i = 0; i < this.fftSize / 2; i++) {
                const freq = i * freqStep;
                let gain = 1.0;
                for (let j = 1; j < profilePoints.freq.length; j++) {
                    if (freq >= profilePoints.freq[j - 1] && freq <= profilePoints.freq[j]) {
                        ...
                        gain = (1 - t) * profilePoints.gain[j - 1] + t * profilePoints.gain[j]; // Linear interpolation
                    }
                }
                const emotionalVector = this.at2030Params.emotionalVector;
                timbreCurve[i] = ensureFinite(gain * emotionalVector, 1.0);
            }
            return timbreCurve;
        } catch (error) {
            handleError("EmotionTimbreMap computation failed", error, { fftSize: this.fftSize });
            return new Float32Array(this.fftSize / 2).fill(1.0);
        }
    }

    processQHP(input, spectralProfile, emotionalMode) {
        try {
            const output = new Float32Array(input.length);
            const { harmonicBoost, transientSculpt, phaseCoherenceFactor } = this.qhp.parameters;
            let order = this.at2030Params.order;
            let sigma = this.at2030Params.sigma;
            let vocalPresence = this.at2030Params.vocalPresence;
            let bassLevel = this.at2030Params.bassLevel;
            let midGain = this.at2030Params.midGain;
            let trebleQ = this.at2030Params.trebleQ;
            let phaseLockFactor = this.at2030Params.phaseLockFactor;
            switch (spectralProfile.profile) {
                case "warm": sigma = 0.4; vocalPresence = 0.9; phaseLockFactor = 0.9; break;
                case "bright": sigma = 0.2; vocalPresence = 0.8; phaseLockFactor = 0.95; break;
                case "bassHeavy": sigma = 0.5; vocalPresence = 0.7; bassLevel = 0.8; phaseLockFactor = 0.85; break;
                case "vocal": sigma = 0.3; vocalPresence = 1.2; phaseLockFactor = 1.0; break;
                case "proNatural": sigma = 0.35; vocalPresence = 1.0; phaseLockFactor = 1.0; break;
                case "karaokeDynamic": sigma = 0.25; vocalPresence = 1.1; phaseLockFactor = 0.95; break;
                case "rockMetal": sigma = 0.45; vocalPresence = 0.85; bassLevel = 0.75; phaseLockFactor = 0.9; break;
                case "smartStudio": sigma = 0.3; vocalPresence = 1.05; phaseLockFactor = 0.95; break;
            }
            let vocalFormant = 0;
            for (let i = Math.floor(200 * this.fftSize / this.sampleRate); i < Math.floor(2000 * this.fftSize / this.sampleRate); i++) {
                vocalFormant += this.analyzers.fft.magnitudes[i];
            }
            vocalFormant = ensureFinite(vocalFormant / (Math.floor(2000 * this.fftSize / this.sampleRate) - Math.floor(200 * this.fftSize / this.sampleRate)));
            vocalFormant *= vocalPresence;
            let Entanglement = Math.pow(Math.abs(vocalFormant * midGain * trebleQ * phaseLockFactor), 0.5);
            if (bassLevel > 0.7) {
                midGain *= 0.95;
                trebleQ *= 0.95;
                Entanglement = Math.pow(Math.abs(vocalFormant * midGain * trebleQ * phaseLockFactor), 0.5);
            }
            let QuantumSuperposition = (f, i) => Math.exp(-Math.pow(f / this.sampleRate, 2) / (2 * Math.pow(sigma, 2))) * Math.cos(2 * Math.PI * (f / this.sampleRate) * i) * (bassLevel * i / order);
            let PhaseCoherence = 0;
            for (let i = 0; i < this.fftSize / 2; i++) {
                let phaseDiff = Math.atan2(this.analyzers.fft.imag[i], this.analyzers.fft.real[i]) - (this.qhp.phaseBuffer[i] || 0);
                PhaseCoherence += Math.cos(phaseDiff) * phaseLockFactor;
            }
            PhaseCoherence = ensureFinite(PhaseCoherence / (this.fftSize / 2));
            let TransientSculpt = (spectralProfile.transientEnergy || 0.5) * this.at2030Params.sculptFactor;
            if (this.devicePerf === "low" && TransientSculpt > 1.0) TransientSculpt *= 0.8;
            for (let i = 0; i < input.length; i += 16) {
                for (let j = 0; j < 16 && i + j < input.length; j++) {
                    ...
                    for (let k = 1; k <= order; k++) {
                        bassGain += QuantumSuperposition(50, k);
                    }
                    sample *= harmonicBoost * bassGain * Entanglement / (PhaseCoherence + TransientSculpt);
                    sample = Math.min(Math.max(sample, this.MIN_SIGNAL_VALUE), this.MAX_SIGNAL_VALUE);
                    if (spectralProfile.profile === "bassHeavy" || spectralProfile.transientEnergy > 0.8) {
                        sample *= 1.1;
                    }
                    if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
                        sample = this.TANH_LUT[Math.floor((sample + 5) * 99.9)] * 0.5 + sample * 0.5;
                    }
                    output[idx] = ensureFinite(sample, this.MAX_SIGNAL_VALUE);
                }
            }
            if (spectralProfile.profile === "bassHeavy" || spectralProfile.profile === "rockMetal") {
                for (let i = 0; i < input.length && i < this.fftSize; i++) {
                    const delta = Math.abs(input[i] - (input[i - 1] || 0));
                    if (delta > 0.15 * transientSculpt) {
                        this.qhp.transientSculptBuffer[i % this.fftSize] = input[i] * transientSculpt;
                        output[i] = ensureFinite(output[i] + this.qhp.transientSculptBuffer[i % this.fftSize] * 0.2);
                    }
                }
            }
            if (spectralProfile.profile === "bright" || spectralProfile.profile === "smartStudio") {
                for (let i = 0; i < this.fftSize; i++) {
                    const phase = Math.atan2(this.analyzers.fft.imag[i], this.analyzers.fft.real[i]);
                    this.qhp.phaseBuffer[i] = phase * phaseCoherenceFactor + (1 - phaseCoherenceFactor) * (Math.random() - 0.5) * 0.1;
                }
            }
            if (emotionalMode === "calm" && spectralProfile.profile === "warm") {
                for (let i = 0; i < output.length; i++) {
                    output[i] *= 0.9;
                }
            } else if (emotionalMode === "aggressive" && spectralProfile.profile === "rockMetal") {
                for (let i = 0; i < output.length; i++) {
                    output[i] *= 1.1;
                }
            }
            ...
            handleError("QHP processing failed", error, { fftSize: this.fftSize, inputLength: input.length });
            return input;
        }
    }

    applyAPEQ(input, spectralProfile) {
        try {
            const output = new Float32Array(input.length);
            const eqGains = this.memoryManager.allocate(this.fftSize / 2);
            const freqStep = this.sampleRate / this.fftSize;
            let formantScale = this.at2030Params.formantScale;
            let pitchShift = this.at2030Params.pitchShift;
            let bassLevel = this.at2030Params.bassLevel;
            let sigma = this.at2030Params.sigma;
            let emotionalVector = this.at2030Params.emotionalVector;
            switch (spectralProfile.profile) {
                case "warm": formantScale = 0.95; sigma = 0.4; emotionalVector = 0.9; break;
                case "bright": formantScale = 1.05; sigma = 0.2; emotionalVector = 1.0; break;
                case "bassHeavy": formantScale = 1.1; sigma = 0.5; emotionalVector = 1.05; break;
                case "vocal": formantScale = 1.0; sigma = 0.3; emotionalVector = 1.0; break;
                case "proNatural": formantScale = 1.0; sigma = 0.35; emotionalVector = 1.0; break;
                case "karaokeDynamic": formantScale = 1.05; sigma = 0.25; emotionalVector = 1.1; break;
                case "rockMetal": formantScale = 1.1; sigma = 0.45; emotionalVector = 1.1; break;
                case "smartStudio": formantScale = 1.0; sigma = 0.3; emotionalVector = 1.05; break;
            }
            if (bassLevel > 0.7) {
                formantScale *= 0.95;
            }
            let MasterFormantScale = formantScale * (1 + pitchShift * 0.03);
            MasterFormantScale = Math.min(Math.max(MasterFormantScale, 0.8), 1.2);
            let DeviceAdaptFactor = Math.max(0.7, Math.min(1.0, 1.0 - ((this.devicePerf === "low" ? 0.9 : this.devicePerf === "medium" ? 0.6 : 0.3) * 0.3)));
            let spectralAttention = this.computeSpectralAttention(this.analyzers.fft.magnitudes, spectralProfile);
            let psychoacousticWeight = this.computePsychoacousticWeight(spectralProfile);
            let timbreCurve = this.computeEmotionTimbreMap(spectralProfile);
            let waveletCoeff = (f, t) => Math.exp(-Math.pow(f / this.sampleRate, 2) / (2 * Math.pow(sigma, 2))) * Math.cos(2 * Math.PI * (f / this.sampleRate) * t);
            for (let i = 0; i < this.fftSize / 2; i++) {
                const freq = i * freqStep;
                let gain = 1.0;
                if (spectralProfile.profile === "vocal" && freq >= 200 && freq <= 2000) {
                    gain = 1.2;
                } else if (spectralProfile.profile === "bassHeavy" && freq <= 100) {
                    gain = 1.1;
                } else if (spectralProfile.profile === "bright" && freq >= 4000 && freq <= 8000) {
                    gain = 0.9;
                }
                let t = i / (this.fftSize / 2);
                let boost = waveletCoeff(freq, t);
                eqGains[i] = ensureFinite(gain * MasterFormantScale * boost * DeviceAdaptFactor * spectralAttention[i] * psychoacousticWeight[i] * timbreCurve[i]);
            }
            const fftResult = await this.fft.fft(input, { type: "white", confidence: 0.5 }, suppressArtifacts);
            for (let i = 0; i < this.fftSize / 2; i++) {
                fftResult[i * 2] *= eqGains[i];
                fftResult[i * 2 + 1] *= eqGains[i];
            }
            const ifftResult = await this.fft.ifft(fftResult);
            for (let i = 0; i < input.length; i++) {
                output[i] = ensureFinite(ifftResult[i], this.MAX_SIGNAL_VALUE);
            }
            this.memoryManager.free(eqGains);
            return output;
        } catch (error) {
            handleError("APEQ processing failed", error, { fftSize: this.fftSize });
            return input;
        }
    }

    preservePitch(input, spectralProfile, pitchInfo) {
        try {
            const output = new Float32Array(input.length);
            const pitches = detectPolyphonicPitches(
                this.analyzers.fft.magnitudes,
                this.fftSize,
                this.sampleRate,
                spectralProfile,
                this.memoryManager,
                { type: "white", confidence: 0.5 },
                pitchInfo,
                suppressArtifacts
            ).pitches;
            ...
            let emotionalVector = this.at2030Params.emotionalVector;
            switch (spectralProfile.profile) {
                case "warm": phaseLockFactor = 0.9; formantScale = 0.95; emotionalVector = 0.9; break;
                case "bright": phaseLockFactor = 0.95; formantScale = 1.05; emotionalVector = 1.0; break;
                case "bassHeavy": phaseLockFactor = 0.85; formantScale = 1.1; emotionalVector = 1.05; break;
                case "vocal": phaseLockFactor = 1.0; formantScale = 1.0; emotionalVector = 1.0; break;
                case "proNatural": phaseLockFactor = 1.0; formantScale = 1.0; emotionalVector = 1.0; break;
                case "karaokeDynamic": phaseLockFactor = 0.95; formantScale = 1.05; emotionalVector = 1.1; break;
                case "rockMetal": phaseLockFactor = 0.9; formantScale = 1.1; emotionalVector = 1.1; break;
                case "smartStudio": phaseLockFactor = 0.95; formantScale = 1.0; emotionalVector = 1.05; break;
            }
            if (this.at2030Params.bassLevel > 0.7) {
                formantScale *= 0.95;
            }
            let PhaseCoherence = 0;
            for (let i = 0; i < this.fftSize / 2; i++) {
            ...
            PhaseCoherence = ensureFinite(PhaseCoherence / (this.fftSize / 2));
            let Entanglement = Math.pow(Math.abs(vocalPresence * this.at2030Params.midGain * this.at2030Params.trebleQ), 0.5);
            if (this.at2030Params.bassLevel > 0.7) {
                this.at2030Params.midGain *= 0.95;
                this.at2030Params.trebleQ *= 0.95;
                Entanglement = Math.pow(Math.abs(vocalPresence * this.at2030Params.midGain * this.at2030Params.trebleQ), 0.5);
            }
            let MasterFormantScale = formantScale * (1 + this.at2030Params.pitchShift * 0.03);
            MasterFormantScale = Math.min(Math.max(MasterFormantScale, 0.8), 1.2);
            let spectralAttention = this.computeSpectralAttention(this.analyzers.fft.magnitudes, spectralProfile);
            let psychoacousticWeight = this.computePsychoacousticWeight(spectralProfile);
            let timbreCurve = this.computeEmotionTimbreMap(spectralProfile);
            if (spectralProfile.profile === "vocal" || spectralProfile.vocalPresence > 0.7) {
                const fftResult = await this.fft.fft(input, { type: "white", confidence: 0.5 }, suppressArtifacts);
                for (let i = 0; i < this.fftSize; i++) {
                    this.at2030Buffer[i] = fftResult[i];
                }
                for (let k = 0; k < pitches.length; k++) {
                    let freqIdx = Math.floor(pitches[k].frequency * this.fftSize / this.sampleRate);
                    if (freqIdx < this.fftSize / 2) {
                        let scale = MasterFormantScale * vocalPresence * PhaseCoherence * Entanglement * emotionalVector;
                        for (let i = 0; i < this.fftSize / 2; i++) {
                            scale *= spectralAttention[i] * psychoacousticWeight[i] * timbreCurve[i];
                        }
                        scale = Math.min(Math.max(scale, 0.8), 1.2);
                        this.at2030Buffer[freqIdx * 2] *= scale;
                        this.at2030Buffer[freqIdx * 2 + 1] *= scale;
                    }
                }
                const ifftResult = await this.fft.ifft(this.at2030Buffer);
                for (let i = 0; i < input.length; i++) {
                    output[i] = ensureFinite(ifftResult[i], this.MAX_SIGNAL_VALUE);
                }
            } else {
            ...
            handleError("Pitch preservation failed", error, { fftSize: this.fftSize });
            return input;
        }
    }

    async processAudio(input, options = {}) {
        const { spectralProfile = { devicePerf: this.devicePerf, profile: "smartStudio" }, isVocal = false, currentGenre = "Unknown", noiseInfo = { type: "white", confidence: 0.5 }, pitchInfo = {} } = options;
        try {
            if (!Number.isFinite(this.at2030Params.bassLevel) || !Number.isFinite(this.at2030Params.vocalPresence)) {
                throw new Error("Invalid AT2030 parameters");
            }
            // Cập nhật listenerProfile và timbreProfile từ options (nếu có)
            this.at2030Params.listenerProfile = options.listenerProfile || this.at2030Params.listenerProfile;
            this.at2030Params.timbreProfile = spectralProfile.profile || this.at2030Params.timbreProfile;
            this.analyzers.fft.initWindow(spectralProfile);
            this.analyzeContent(input, spectralProfile);
            const preset = {
                threshold: spectralProfile.profile === "bassHeavy" ? -18 : spectralProfile.profile === "vocal" ? -22 : -20,
                ratio: spectralProfile.profile === "vocal" ? 4 : 5,
                makeupGain: spectralProfile.profile === "warm" ? 1.2 : 1.0,
                transientAmount: spectralProfile.profile === "bassHeavy" ? 1.5 : 1.0,
                cochlearCompression: spectralProfile.profile === "vocal" ? 0.8 : 0.5,
                entropyColorationFactor: spectralProfile.profile === "rockMetal" ? 0.3 : 0.2,
                breathingDepth: spectralProfile.profile === "warm" ? 0.3 : 0.1,
                artifactAmount: spectralProfile.profile === "rockMetal" ? 0.2 : 0.1,
                enableControlledArtifacts: spectralProfile.profile === "rockMetal" || spectralProfile.profile === "smartStudio"
            };
            this.applyCompressorPreset(preset);
            this.qhp.parameters.harmonicBoost = spectralProfile.profile === "bassHeavy" ? 1.3 : spectralProfile.profile === "vocal" ? 1.1 : 1.0;
            this.qhp.parameters.transientSculpt = spectralProfile.profile === "bassHeavy" || spectralProfile.profile === "rockMetal" ? 1.5 : 1.0;
            this.qhp.parameters.phaseCoherenceFactor = spectralProfile.profile === "bright" || spectralProfile.profile === "smartStudio" ? 0.9 : 0.8;
            this.qhp.parameters.emotionalModeFactor = this.contentAnalyzer.emotionalMode === "calm" ? 0.9 : this.contentAnalyzer.emotionalMode === "aggressive" ? 1.1 : 1.0;
            const freqData = await this.analyzers.fft.process(input, spectralProfile, noiseInfo);
            const transientEnergy = this.analyzers.transient.process(input, spectralProfile);
            let cleanInput = this.analyzers.noiseSuppressor.process(input, spectralProfile, noiseInfo);
            let order = this.at2030Params.order;
            let sigma = this.at2030Params.sigma;
            let vocalPresence = this.at2030Params.vocalPresence;
            let bassLevel = this.at2030Params.bassLevel;
            ...
            let pitchShift = this.at2030Params.pitchShift;
            switch (spectralProfile.profile) {
                case "warm": sigma = 0.4; vocalPresence = 0.9; phaseLockFactor = 0.9; emotionalVector = 0.9; formantScale = 0.95; break;
                case "bright": sigma = 0.2; vocalPresence = 0.8; phaseLockFactor = 0.95; emotionalVector = 1.0; formantScale = 1.05; break;
                case "bassHeavy": sigma = 0.5; vocalPresence = 0.7; bassLevel = 0.8; phaseLockFactor = 0.85; emotionalVector = 1.05; formantScale = 1.1; break;
                case "vocal": sigma = 0.3; vocalPresence = 1.2; phaseLockFactor = 1.0; emotionalVector = 1.0; formantScale = 1.0; break;
                case "proNatural": sigma = 0.35; vocalPresence = 1.0; phaseLockFactor = 1.0; emotionalVector = 1.0; formantScale = 1.0; break;
                case "karaokeDynamic": sigma = 0.25; vocalPresence = 1.1; phaseLockFactor = 0.95; emotionalVector = 1.1; formantScale = 1.05; break;
                case "rockMetal": sigma = 0.45; vocalPresence = 0.85; bassLevel = 0.75; phaseLockFactor = 0.9; emotionalVector = 1.1; formantScale = 1.1; break;
                case "smartStudio": sigma = 0.3; vocalPresence = 1.05; phaseLockFactor = 0.95; emotionalVector = 1.05; formantScale = 1.0; break;
            }
            if (bassLevel > 0.7) {
                midGain *= 0.95;
                trebleQ *= 0.95;
            ...
            for (let i = 1; i <= order; i++) {
                bassGain += QuantumSuperposition(50, i);
            }
            let vocalFormant = 0;
            for (let i = Math.floor(200 * this.fftSize / this.sampleRate); i < Math.floor(2000 * this.fftSize / this.sampleRate); i++) {
                vocalFormant += this.analyzers.fft.magnitudes[i];
            ...
            vocalFormant = ensureFinite(vocalFormant / (Math.floor(2000 * this.fftSize / this.sampleRate) - Math.floor(200 * this.fftSize / this.sampleRate)));
            vocalFormant *= vocalPresence;
            let Entanglement = Math.pow(Math.abs(vocalFormant * midGain * trebleQ), 0.5);
            let PhaseCoherence = 0;
            for (let i = 0; i < this.fftSize / 2; i++) {
                let phaseDiff = Math.atan2(this.analyzers.fft.imag[i], this.analyzers.fft.real[i]) - (this.qhp.phaseBuffer[i] || 0);
                PhaseCoherence += Math.cos(phaseDiff) * phaseLockFactor;
            ...
            PhaseCoherence = ensureFinite(PhaseCoherence / (this.fftSize / 2));
            let TransientSculpt = transientEnergy * this.at2030Params.sculptFactor;
            if (this.devicePerf === "low" && TransientSculpt > 1.0) TransientSculpt *= 0.8;
            let AIEmotionalAdjust = emotionalVector * this.qhp.parameters.emotionalModeFactor;
            let MasterFormantScale = formantScale * (1 + pitchShift * 0.03);
            MasterFormantScale = Math.min(Math.max(MasterFormantScale, 0.8), 1.2);
            let spectralAttention = this.computeSpectralAttention(freqData, spectralProfile);
            let psychoacousticWeight = this.computePsychoacousticWeight(spectralProfile);
            let timbreCurve = this.computeEmotionTimbreMap(spectralProfile);
            let atGain = (bassGain * Entanglement) / (PhaseCoherence + TransientSculpt) * AIEmotionalAdjust * MasterFormantScale;
            for (let i = 0; i < this.fftSize / 2; i++) {
                atGain *= spectralAttention[i] * psychoacousticWeight[i] * timbreCurve[i];
            }
            atGain = Math.min(Math.max(atGain, 0.8), 1.2);
            for (let i = 0; i < cleanInput.length; i++) {
                cleanInput[i] *= atGain;
                cleanInput[i] = ensureFinite(cleanInput[i], this.MAX_SIGNAL_VALUE);
            }
            cleanInput = this.compressCLA3A(cleanInput, spectralProfile);
            cleanInput = this.processQHP(cleanInput, spectralProfile, this.contentAnalyzer.emotionalMode);
            cleanInput = this.applyAPEQ(cleanInput, spectralProfile);
            cleanInput = this.preservePitch(cleanInput, spectralProfile, pitchInfo);
            ...
            const result = {
                ...
                data: {
                    ...
                    isVocal,
                    currentGenre,
                    emotionalMode: this.contentAnalyzer.emotionalMode,
                    compressorState: this.getCompressorState(),
                    qhpParameters: { ...this.qhp.parameters },
                    at2030Params: { ...this.at2030Params }
                }
            };
            self.postMessage(result);
            return cleanInput;
        } catch (error) {
            handleError("Audio processing failed", error, { fftSize: this.fftSize, inputLength: input.length });
            self.postMessage({ type: "analyzerError", error: `Audio processing failed: ${error.message}` });
            throw error;
        }
    }
    ...
    dispose() {
        try {
            ...
            this.memoryManager.free(this.analyzers.transient.history);
            this.memoryManager.free(this.analyzers.noiseSuppressor.noiseProfile);
            ...
            this.fft.dispose();
            console.debug(`AudioAnalyzer disposed for fftSize ${this.fftSize}`);
        } catch (error) {
            handleError("Failed to dispose AudioAnalyzer", error, { fftSize: this.fftSize });
        }
    }
}
	...
	function runUnitTests() {
		...
		const {
			magnitudes,
			phases
		} = getMagnitudeAndPhase(fftData, 1024);
		...
		// Test Wiener Gain
		const wienerGain = calculateWienerGain(profile, {
			type: "white",
			confidence: 0.5
		}, {
			pitch: 440,
		...
		const result = audioAnalyzer.processAudio(testSignal, {
