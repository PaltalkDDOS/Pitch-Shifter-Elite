...
function updateIcon(tabId, enabled) {
	chrome.tabs.get(tabId, (tab) => {
		if (chrome.runtime.lastError || !tab) {
			console.warn(`Tab ${tabId} not found, skipping icon update.`);
			return;
		}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	...
	switch (message.type) {
		case "ping":
			...
			return false; // Đồng bộ, không async
		case "checkFirstRunAfterUpdate":
		...
		case "get_tab_status":
			chrome.storage.local.get(["activeTab", "tabStates"], (result) => {
				if (chrome.runtime.lastError) {
					logError("Lỗi khi lấy storage cho get_tab_status", chrome.runtime.lastError, {
						tabId
					});
					sendResponse({
						status: "error",
				...
				sendMessageToTabDebounced(tabId, {
					...
					if (response?.status === "success") {
						...
						logError("Lỗi khi lấy trạng thái tab", null, {
							tabId,
							response
						});
						sendResponse({
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        if (!isSupportedUrl(tabUrl)) {
            logError("Active tab URL not supported", new Error("Unsupported URL"), { tabId, url: tabUrl, message });
            sendResponse({ status: "error", message: "Active tab URL not supported" });
            return;
        }

        if (tab.status !== "complete") {
            logError("Active tab not fully loaded", new Error("Tab loading"), { tabId, url: tabUrl, message });
...
chrome.runtime.onInstalled.addListener((details) => {
	...
	if (details.reason === "install") {
		...
		chrome.storage.local.set({
			version: manifest.version,
			lastVersion: manifest.version // ← Dòng quan trọng nhất
		});
	}
});
